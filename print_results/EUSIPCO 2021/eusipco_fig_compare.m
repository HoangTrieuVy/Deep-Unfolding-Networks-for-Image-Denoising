...
subplot(3,8,4);imagesc(resim_dncnn);axis image off; title({dncnn;['batchsize:',num2str(batchsize)];['nb param:', num2str(nb_dncnn),' F:',num2str(F),' K:',num2str(K)];['std:',num2str(std_noise)];['PSNR: ' num2str(plpsnr(double(clean),resim_dncnn),'%3.2f') '/ SSIM: ' num2str(ssim(double(clean),resim_dncnn),'%3.2f')]} );
subplot(3,8,5);imagesc(resim_ista);axis image off; title({ista;['batchsize:',num2str(batchsize)];['nb param:', num2str(nb_ista),' F:',num2str(F),' K:',num2str(K)];['std:',num2str(std_noise)];['PSNR: ' num2str(plpsnr(double(clean),resim_ista),'%3.2f') '/ SSIM: ' num2str(ssim(double(clean),resim_ista),'%3.2f')]} );
% subplot(3,8,6);imagesc(resim_fista);axis image off; title({fista;['batchsize:',num2str(batchsize)];['nb param:', num2str(nb_fista),' F:',num2str(F),' K:',num2str(K)];['std:',num2str(std_noise)];['PSNR: ' num2str(plpsnr(double(clean),resim_fista),'%3.2f') '/ SSIM: ' num2str(ssim(double(clean),resim_fista),'%3.2f')]} );
% subplot(3,8,7);imagesc(resim_cpv3);axis image off; title({cpv2;['batchsize:',num2str(batchsize)];['nb param:', num2str(nb_cpv3),' F:',num2str(F),' K:',num2str(K)];['std:',num2str(std_noise)];['PSNR: ' num2str(plpsnr(double(clean),resim_cpv3),'%3.2f') '/ SSIM: ' num2str(ssim(double(clean),resim_cpv3),'%3.2f')]} );
subplot(3,8,8);imagesc(resim_cpv2);axis image off; title({cpv2;['batchsize:',num2str(batchsize)];['nb param:', num2str(nb_cpv2),' F:',num2str(F),' K:',num2str(K)];['std:',num2str(std_noise)];['PSNR: ' num2str(plpsnr(double(clean),resim_cpv2),'%3.2f') '/ SSIM: ' num2str(ssim(double(clean),resim_cpv2),'%3.2f')]} );
...
h=hsv(18);
figure(100)
semilogy(PSNR_dncnn,'Color',h(1,:));hold on;
semilogy(PSNR_ista,'Color',h(4,:));hold on;
semilogy(PSNR_cpv2,'Color',h(8,:));hold on;

legend({dncnn,ista,cpv2},'Location','southeast')
title('PNSR test');
ylim([10,27]);
set(gca,'FontSize',17)
xlabel('Epochs')
...
figure(101)
load(['../src/out/',dncnn,name_im,'.jpg','_',dncnn,'_F',num2str(F_dncnn),'_K',num2str(K_dncnn),'_batchsize',num2str(batchsize),'_epochs_',num2str(epoch),'.mat']);
plot(eval_dncnn,'Color',h(1,:));hold on;
plot(eval_ista,'Color',h(4,:));hold on;
plot(eval_cpv2,'Color',h(8,:));hold on;
